...
```text
gen-sample ──▶ inspect ──▶ explain-blocking ──▶ recall ──▶ estimate ──▶ predict ──▶ cluster
   (or your      sanity      is the plan          does the    model.json    edges/     clusters.csv
    own data)     check       affordable?          plan reach
                                                   the matches?
```
...
The first four are diagnostics and cost seconds. They exist because the two decisions that
