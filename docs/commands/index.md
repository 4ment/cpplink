...
```text
gen-sample ──▶ inspect ──▶ profile ──▶ explain-blocking ──▶ recall ──▶ estimate ──▶ predict ──▶ cluster
   (or your      sanity      is there    is the plan          does the    model.json    edges/     clusters.csv
    own data)     check      anything    affordable?          plan reach
                             to find?                          the matches?
```
...
The first five are diagnostics and cost seconds. They exist because the two decisions that
...
plan and no known pairs, just the columns.
