...
from transactions import (  # noqa: E402
    CPPLINK_TRACKS, DESTINATION, MATCHED_KEYS, ORIGIN, RESULTS, ROWS, SPLINK_TRACKS,
    THRESHOLDS, TRACKS, schema_path,
)
...
RUNNERS = {"cpplink": os.path.join(HERE, "run_cpplink.py"),
...
def verify():
    ...
    """
    text = subprocess.run(
        [BINARY, "explain-blocking", "--schema", schema_path("matched"), "--count",
         ORIGIN, DESTINATION], capture_output=True, text=True, check=True).stdout
    ours = {}
    ...
    theirs = json.loads(subprocess.run(
        [PYTHON, RUNNERS["splink"], "--track", "matched", "--out",
         os.path.join(RESULTS, "verify"), "--count-comparisons"],
        capture_output=True, text=True, check=True).stdout)
    agree = True
    for key in MATCHED_KEYS:
        same = ours.get(key) == theirs.get(key)
        agree &= same
        print(f"  {key:<18} cpplink {ours.get(key):>12,}   splink {theirs.get(key):>12,}"
              f"   {'ok' if same else 'MISMATCH'}")
    return agree


def run_once(tool, track, out, threads, repeat_index):
    command = [PYTHON, RUNNERS[tool], "--track", track, "--out", out,
               "--threads", str(threads), "--thresholds", THRESHOLDS]
...
def main():
    ...
    if args.verify:
        ok = verify()
        print("  matched blocking agrees" if ok else "  MISMATCH: the matched track is not matched")
    ...
    for track in args.tracks.split(","):
        for tool in args.tools.split(","):
            ...
            for repeat in range(args.repeat):
                ...
                print(f"  repeat {repeat}: {reports[-1]['pipeline_seconds']:.2f} s, "
                      f"{human_bytes(reports[-1]['peak_rss_bytes'])}", flush=True)
            first = reports[0]
            run = {
                "tool": tool, "track": track,
                "pipeline_seconds": statistics.median(r["pipeline_seconds"] for r in reports),
                # The analysis stages run on the first repeat only.
                "stages": {k: statistics.median(r["stages"][k] for r in reports)
    ...
    for run in summary["runs"]:
        ...
        print(f"| {run['track']} | {run['tool']} | {candidates} | {recall} "
